...
def ensure_database() -> None:
    """Create the configured local database when it does not exist (skipped for cloud DBs)."""
    if settings.is_cloud_database:
        logger.info("Cloud PostgreSQL detected (%s); skipping CREATE DATABASE.", settings.postgres_host)
        return
    try:
        with psycopg.connect(
        ...
        logger.warning("Could not execute CREATE DATABASE check (non-fatal): %s", exc)
