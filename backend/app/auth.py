...
def seed_initial_users(db: Session) -> None:
    ...
    accounts = [
        (
            getattr(settings, "INITIAL_ADMIN_USER_ID", "admin"),
            getattr(settings, "INITIAL_ADMIN_PASSWORD", "SecurePitch2026!"),
    ...
    for configured_user_id, password, role in accounts:
        ...
        try:
            ...
            logger.warning("[startup] Skipping seed user %r due to password policy: %s", user_id, err)
            continue
        db.add(User(user_id=user_id, password_hash=hash_password(password), role=role))
        changed = True
        logger.info("[startup] Seeded initial user account: %r (%s)", user_id, role)
    changed = bool(
