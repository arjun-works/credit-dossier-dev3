...
class Settings(BaseSettings):
    ...
    # Initial local accounts. Override these values in backend/.env or deployment settings.
    INITIAL_ADMIN_USER_ID: str = "admin"
    INITIAL_ADMIN_PASSWORD: str = "SecurePitch2026!"
    INITIAL_RELATIONSHIP_MANAGER_USER_ID: str = "manager"
    INITIAL_RELATIONSHIP_MANAGER_PASSWORD: str = "SecurePitch2026!"
    INITIAL_CREDIT_ANALYST_USER_ID: str = "analyst"
    INITIAL_CREDIT_ANALYST_PASSWORD: str = "SecurePitch2026!"

    @field_validator("DATABASE_URL")
    @classmethod
    def require_postgresql(cls, value: str) -> str:
        if not value.startswith(("postgresql://", "postgresql+", "postgres://")):
            raise ValueError("DATABASE_URL must use PostgreSQL.")
        # Automatically map standard postgres:// and postgresql:// URLs to postgresql+psycopg://
        # so SQLAlchemy 2.0 uses psycopg (v3) instead of searching for psycopg2
